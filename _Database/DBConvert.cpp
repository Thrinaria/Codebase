...
//			if( nIndex < PARTS_CLOAK2+1 )
		...
		if( IsAbnormalPlayerData( pMover ) )
		{
			...
			*sEquipment		= '\0';
			for( nIndex = 0; nIndex < PARTS_CLOAK2+1; nIndex++ )
			{
				char s[32]	= { 0, };
...
//			continue;
		...
		while( '$' != sInventory[nOffset] )
		{
			...
			{
				if( nId >= MAX_INVENTORY + PARTS_CLOAK2+1)
				{
...
BOOL CDbManager::SN( void )
{
	...
	while( pQueryChar->Fetch() )
	{
		...
		{
			if( IndexEquipment < PARTS_CLOAK2+1 )
		...
		for( DWORD dwParts	= 0; dwParts < PARTS_CLOAK2+1; dwParts++ )
		{
