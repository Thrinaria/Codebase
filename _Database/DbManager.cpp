...
//			qry->Clear();
...

		{
			...
			while( '$' != Inven[CountStr] )
			{
				...
				{
					if( IndexItem >= MAX_INVENTORY + PARTS_CLOAK2+1 )
					{
			...
			int countbuf = 0;
			CItemElem* pItemElem;
			for( int i = 0; i < PARTS_CLOAK2+1; i++ )
			{
...
void CDbManager::UpdateGuildNotice( CQuery* pQuery, LPDB_OVERLAPPED_PLUS lpDbOverlappedPlus )
{
	...
	if( pQuery->BindParameter( 1, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_VARCHAR, 1024, 0, szNotice, 0, &cbLen ) == FALSE )
	{
		FreeRequest( lpDbOverlappedPlus );
		return;
	}
