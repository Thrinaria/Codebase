...
//	virtual int		OnActCollecting();				// User만 사용되는 것이므로 CUser가서 찾을것.
	...
	void			SetMode( DWORD dwMode )		{ m_dwMode |= dwMode; }	// 유저상태 V팅
	void			SetNotMode( DWORD dwMode )	{ m_dwMode &= (~dwMode); } // 유저상태 리V
	...
	void			SetStateMode( DWORD dwMode, BYTE nFlag );		// 유저상태 V팅
	void			SetStateNotMode( DWORD dwMode, BYTE nFlag );	// 유저상태 리V
