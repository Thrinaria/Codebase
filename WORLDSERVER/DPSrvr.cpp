...
void CDPSrvr::OnChangeBankPass( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize )
{
	...
	if( IsValidObj( pUser ) )
	{
		...
		{
			// 패스워드가 바꿨으므로 DB와 클라이언트에 게 바다고 보내줌
...
void CDPSrvr::OnSendTeleportPosition( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long )//Teleport Fenster Stuff
{
    ...
    if( IsValidObj( pUser ) )
        ...
        {
            case 1:
                vPos = D3DXVECTOR3(7161.0F, 100.0F, 3264.0F); dwWorldID = 1; //flaris
            break;
            case 2:
                vPos = D3DXVECTOR3(8321.0F, 100.0F, 3720.0F); dwWorldID = 1; //stmorning
            break;
            case 3:
                vPos = D3DXVECTOR3(3896.0F, 59.0F, 4378.0F); dwWorldID = 1; //darkon
            break;
            case 4:
                vPos = D3DXVECTOR3(5936.0F, 100.0F, 5421.0F); dwWorldID = 1; //shaduwar
            break;
            case 5:
                vPos = D3DXVECTOR3(6065.0F, 70.0F, 9234.0F); dwWorldID = 1; //Desert
            break;
            case 6:
                vPos = D3DXVECTOR3(7235.0F, 66.0F, 9093.0F); dwWorldID = 1; //Elluin
            break;
            case 7:
                vPos = D3DXVECTOR3(828.0F, 100.0F, 836.0F); dwWorldID = 131; //Herneos 1
            break;
            case 8:
                vPos = D3DXVECTOR3(828.0F, 100.0F, 836.0F); dwWorldID = 134; //Herneos 2
            break;
            case 9:
                vPos = D3DXVECTOR3(717.0F, 100.0F, 230.0F); dwWorldID = 132; //Sanpres 1
            break;
            case 10:
                vPos = D3DXVECTOR3(717.0F, 100.0F, 230.0F); dwWorldID = 135; //Sanpres 2
            break;
            case 11:
                vPos = D3DXVECTOR3(1176.0F, 150.0F, 650.0F); dwWorldID = 129; //Upresia 1
            break;
            case 12:
                vPos = D3DXVECTOR3(1176.0F, 150.0F, 650.0F); dwWorldID = 133; //Upresia 2
            break;
//            return FALSE;
        }
		...
		case 1: //Start Match
			...
			if( pParty && pParty->IsLeader( pUser->m_idPlayer ) )
			{
				if( CColosseumMng::GetInstance()->CreateNewMatch( pUser, byDifficult, byMenu ) )
				{
					...
					pUser->AddText("Match has started");
				}
				else
					pUser->AddText( "Currently not possible." );
			}
			else
				pUser->AddText( "You have no party or not the party leader." );
