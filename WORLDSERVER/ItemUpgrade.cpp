...
void CItemUpgrade::LoadScript()
{
	...
	CLuaBase	bar;
	if( bar.RunScript( "Baruna.lua" ) != 0 )
	{
		Error( "CItemUpgrade::LoadScript() - Baruna.lua Run Failed!!!" );
		ASSERT(0);
	}

	bar.GetGloabal( "tBarunaUpgrade" );
	bar.PushNil();
	while( bar.TableLoop( -2 ) )
	{
		m_mapBarunaUp.insert( make_pair( static_cast<int>(bar.ToNumber(-2)), static_cast<int>(bar.ToNumber(-1)) ) );
		bar.Pop( 1 );
	}
	bar.Pop(0);

	bar.GetGloabal( "tBarunaDowngrade" );
	bar.PushNil();
	while( bar.TableLoop( -2 ) )
	{
		m_mapBarunaDown.insert( make_pair( static_cast<int>(bar.ToNumber(-2)), static_cast<int>(bar.ToNumber(-1)) ) );
		bar.Pop( 1 );
	}
	bar.Pop(0);
...
BYTE	CItemUpgrade::OnSmeltSafety( CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr, CItemElem* pItemSmeltScr )
{
	...
	switch( pItemMaterial->GetProp()->dwItemKind3 )
	{
		...
		case IK3_OPERCID:
			if( pItemMain->GetProp()->IsBaruna() )
				return BarunaSafe( pUser, pItemMain, pItemMaterial, pItemProtScr );
...
BYTE	CItemUpgrade::BarunaSafe(CUser* pUser, CItemElem* pItemMain, CItemElem* pItemMaterial, CItemElem* pItemProtScr )
{
	bool noDown = false;
...
//	}
	...
	if( pItemProtScr->m_dwItemId != II_SYS_SYS_SCR_BARUNASMELPROT &&
		pItemProtScr->m_dwItemId != II_SYS_SYS_SCR_BARUNASMELPROT02 )
	{
		return 0;
	}

	if( pItemProtScr->m_dwItemId == II_SYS_SYS_SCR_BARUNASMELPROT02 )
	{
		noDown = true;
	}
	...
	int nProb;
	int	dProb;
	int nAbilityOption = pItemMain->GetAbilityOption();

	map<int, int>::iterator it = m_mapBarunaUp.find( nAbilityOption + 1 );
	if( it != m_mapBarunaUp.end() )
		nProb = it->second;
	it = m_mapBarunaDown.find( nAbilityOption + 1 );
	if( it != m_mapBarunaDown.end() )
		dProb = it->second;

	if( nProb < (int)( xRandom( 10000 ) ) )
	{	
		pUser->AddPlaySound( SND_INF_UPGRADEFAIL );
		if( pUser->IsMode( TRANSPARENT_MODE ) == 0)
			g_UserMng.AddCreateSfxObj( (CMover *)pUser, XI_INT_FAIL, pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z);
