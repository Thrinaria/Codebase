...
void CWndCharInfo::OnDraw(C2DRender* p2DRender)
{
	...
	if(checkhero == LEGEND_CLASS_MASTER && m_nCampusPoint == 0) //전승을 했을 경우.
	{
	...
	else if(checkhero == LEGEND_CLASS_HERO && m_nCampusPoint == 0) //영웅일 경우.
	{
		...
		else if(checkhero == LEGEND_CLASS_LEGENDHERO && m_nCampusPoint == 0) //영웅일 경우.
	{
...
{ 
	...
	if(pWndButton)
	{
		if(::GetLanguage() == LANG_ENG || ::GetLanguage() == LANG_PHP ||
			::GetLanguage() == LANG_ID || ::GetLanguage() == LANG_VTN)
