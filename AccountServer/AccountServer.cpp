	...
	if( s.Load( lpszFileName ) )
	{
		...
		while( s.tok != FINISHED )
		{
			...
			else if( s.Token == "AddTail" )
			{
				...
				//strcpy( pServer->lpName, "Heralds of Madrigal");
				//strcpy( pServer->lpAddr, "31.151.61.49" ); // lock AccountServer
				//strcpy( pServer->lpAddr, "122.52.179.57" ); // lock AccountServer
